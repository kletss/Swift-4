...
//    }
    ...
    private let id: Int = Int( MySingletonSession.instance.userID)!
    private lazy var vkGroups = try? RealmService.load(typeOf: RealmGroup.self)
//        .filter(NSPredicate(format: "idUser == %i", id))
   
    private var token: NotificationToken?
 
//       var myGroups = [groupModel]()
    private let animatorUp = PushAnimation1()
    private let animatorDown = PopAnimation1()
...
// //       cell.imageView?.image = myGroups[indexPath.item].image
        photoService.getImage(urlString: filteredGroups[indexPath.item].photoAvatarURL) { [weak self] image in
                self?.avatar = image!
        }
        
        cell.configate(avatar: avatar,
