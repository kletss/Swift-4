...
//    private lazy var fotkiCount = vkPhotos?.count ?? 0
    private var photo: UIImage = UIImage(named: "nonePhoto")!
...
//    private lazy var groupsCount = vkGroups?.count ?? 0
    ...
    private let loadQueueP = OperationQueue()
    
//    private let photoService = PhotoService()
    private let photoServiceP: PhotoService = {
        let appDelegate = UIApplication.shared.delegate as? AppDelegate
        return appDelegate?.photoService ?? PhotoService()
    }()
    private let photoServiceG: PhotoService = {
        let appDelegate = UIApplication.shared.delegate as? AppDelegate
        return appDelegate?.photoService ?? PhotoService()
    }()
    ...
    override func viewDidLoad() {
        ...
        photoServiceP.getImage(urlString: frendAvatarURLVar) { [weak self] image in
...
//                   }
        ...
        operDataPhoto.addDependency(operSaveGroup)
...
//                }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        photoServiceP.getImage(urlString: vkPhotos![indexPath.item].sizes.first!.photoURL) { [weak self] image in
//                   DispatchQueue.main.async {
                    self?.photo = image!
//                   }
            }
...
//            cell.configate(photoUrl: vkPhotos![indexPath.item].sizes.first!.photoURL)
            ...
            photoServiceG.getImage(urlString: vkGroups![indexPath.item].photoAvatarURL) { [weak self] image in
//                   DispatchQueue.main.async {
